...
### A note on per-topic video and picture search links (Science pilot)

As a pilot (Science only, all grades, before deciding whether to extend to other subjects), each of the 169 per-grade Science topics now also gets a `video_resources` entry (a plain YouTube search-results link, e.g. "Pollination for kids") and an `infographics`-array entry (a plain Wikimedia Commons search-results link) for that topic. Following the same precedent already established for World Landmark videos, **no individually-vetted video or image exists for an arbitrary topic like "Doppler effect" or "Cell theory,"** so both are real, live search-result pages rather than fabricated or guessed specific URLs, and both are explicitly marked `"safe": false` with a description disclosing that the link is a general, unvetted search rather than a verified resource — so parents can judge whether to use it with their child. No infographic, audio, or interactive-game entry was added per topic, since no honest, generically-real source for those resource types exists for an arbitrary science topic; inventing one would violate this project's standing no-fabrication rule.

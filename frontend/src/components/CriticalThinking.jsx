    ...
    <div className="mt-5 border-t pt-4">
        ...
        <div key={i} className="mb-3">
          ...
          <div className="space-y-1">
            {q.options.map((opt, j) => {
              ...
              if (!submitted) cls += answers[i] === j ? 'border-violet-500 bg-violet-50' : 'border-gray-200 hover:bg-gray-50';
      ...
      {!submitted ? (
        <button onClick={() => setSubmitted(true)}
          ...
          className="mt-2 px-5 py-2 bg-violet-600 text-white text-sm rounded-lg hover:bg-violet-700 disabled:opacity-50">
          Submit
        </button>
      ) : (
        <div className="mt-2 p-3 rounded-lg bg-violet-50 border border-violet-200 text-center">
          <p className="font-bold text-violet-700">{score}/{quiz.length}</p>
          <button onClick={() => { setAnswers({}); setSubmitted(false); }} className="text-xs text-violet-600 underline mt-1">Retry</button>
        </div>
      )}
    </div>
  );
...
function LessonDetail({ moduleId, lessonId, onBack }) {
  ...
  useEffect(() => {
    fetch(`${API}/critical-thinking/${moduleId}/${lessonId}`).then(r => r.json()).then(setLesson);
  }, [moduleId, lessonId]);
  if (!lesson) return <div className="p-4 text-gray-500">Loading…</div>;
  return (
    <div>
      <button onClick={onBack} className="mb-4 text-sm text-violet-600 hover:underline">← Back</button>
      ...
      {lesson.exercise && (
        <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 mb-4">
          <h3 className="font-semibold text-amber-800 mb-2">✍️ Practice</h3>
...
function ModuleView({ mod, onBack }) {
  ...
  useEffect(() => {
    fetch(`${API}/critical-thinking/${mod.id}`).then(r => r.json()).then(setData);
  }, [mod.id]);
  if (selectedLesson) return <LessonDetail moduleId={mod.id} lessonId={selectedLesson} onBack={() => setSelectedLesson(null)} />;
  return (
    <div>
      <button onClick={onBack} className="mb-3 text-sm text-violet-600 hover:underline">← All Modules</button>
        ...
        <div className="space-y-3">
          {data.lessons?.map((lesson, i) => (
            <button key={lesson.id} onClick={() => setSelectedLesson(lesson.id)}
              className="w-full text-left rounded-xl border-2 border-violet-200 bg-violet-50 p-4 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-violet-500 text-white text-sm flex items-center justify-center font-bold flex-shrink-0">{i + 1}</div>
                <div>
                  <p className="font-semibold text-gray-800">{lesson.title}</p>
                  <p className="text-xs text-violet-600 mt-0.5">{lesson.quiz?.length || 0} quiz questions</p>
                </div>
              </div>
            </button>
          ))}
...
export default function CriticalThinking() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      <h1 className="text-3xl font-bold text-violet-700 mb-1">🧠 Critical Thinking Academy</h1>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {overview.modules.map(mod => (
          <button key={mod.id} onClick={() => setSelectedMod(mod)}
            className="text-left rounded-xl border-2 border-violet-200 bg-gradient-to-br from-violet-50 to-purple-50 p-5 hover:shadow-lg transition-shadow">
            ...
            <p className="text-xs text-violet-600 mt-2">{mod.lesson_count} lesson{mod.lesson_count !== 1 ? 's' : ''}</p>
          </button>
        ))}
      </div>
    </div>

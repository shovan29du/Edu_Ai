  ...
  return (
    <div className="mt-5 border-t pt-4">
        ...
        <div key={i} className="mb-3">
          ...
          <div className="space-y-1">
            {q.options.map((opt, j) => {
              ...
              if (!submitted) cls += answers[i] === j ? 'border-blue-600 bg-blue-50' : 'border-gray-200 hover:bg-gray-50';
      ...
      {!submitted ? (
        ...
        <div className="mt-2 p-3 rounded-lg bg-blue-50 border border-blue-200 text-center">
          <p className="font-bold text-blue-700">{score}/{quiz.length}</p>
    ...
    <div>
      ...
      {lesson.example && (
        <div className="rounded-xl bg-indigo-50 border border-indigo-200 p-4 mb-4">
          <h3 className="font-semibold text-indigo-800 mb-2">🌍 Real-World Example</h3>
          <p className="text-sm text-gray-800 whitespace-pre-line">{lesson.example}</p>
        </div>
      )}
      {lesson.exercise && (
        <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 mb-4">
          <h3 className="font-semibold text-amber-800 mb-2">✍️ Discussion Exercise</h3>
        ...
        <div className="space-y-3">
          {data.lessons?.map((lesson, i) => (
            <button key={lesson.id} onClick={() => setSelectedLesson(lesson.id)}
              className="w-full text-left rounded-xl border-2 border-blue-200 bg-blue-50 p-4 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-600 mt-0.5">{lesson.quiz?.length || 0} quiz questions</p>
                </div>
              </div>
            </button>
          ))}
...
export default function WorldPolitics() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      <h1 className="text-3xl font-bold text-blue-800 mb-1">⚡ World Politics & International Relations</h1>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {overview.modules.map(mod => (
          <button key={mod.id} onClick={() => setSelectedMod(mod)}
            className="text-left rounded-xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-5 hover:shadow-lg transition-shadow">
            ...
            <p className="text-xs text-blue-600 mt-2">{mod.lesson_count} lesson{mod.lesson_count !== 1 ? 's' : ''}</p>
          </button>
        ))}
      </div>
    </div>

...
export default function SongCentre() {
  ...
  async function handleRefreshLinks() {
    ...
    try {
      const res = await fetch(`${API}/songs/refresh-links`, { method: 'POST' });
      const result = await res.json();
      setRefreshResult(result);
      // Reload songs so any fixed links show up
      fetch(`${API}/songs`).then(r => r.json()).then(setData);
    } catch (e) {
      setRefreshResult({ error: 'Failed to connect to server.' });
    } finally {
      setRefreshing(false);
    }
  }
      ...
      <div className="rounded-2xl bg-gradient-to-r from-purple-600 to-violet-700 text-white p-6 mb-6 shadow-lg">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <button
            ...
            disabled={refreshing}
            className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-medium rounded-xl border border-white/30 transition-colors"
          >
            {refreshing ? (
              <><span className="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Checking…</>
            ) : (
              <><span>🔄</span> Update Links</>
            )}
          </button>
        </div>
        ...
        {refreshResult && (
          <div className={`mt-4 rounded-xl p-3 text-sm ${refreshResult.error ? 'bg-red-500/30 text-red-100' : refreshResult.fixed === 0 ? 'bg-green-500/30 text-green-100' : 'bg-amber-500/30 text-amber-100'}`}>
            {refreshResult.error ? (
              <span>❌ {refreshResult.error}</span>
            ) : refreshResult.fixed === 0 ? (
              <span>✅ All {refreshResult.total_checked} song links are healthy — nothing needed fixing.</span>
            ) : (
              <div>
                <p>🔧 Fixed {refreshResult.fixed} of {refreshResult.total_checked} songs — links repaired and saved.</p>
                {refreshResult.fixed_songs?.length > 0 && (
                  <p className="mt-1 text-xs opacity-80">
                    Repaired: {refreshResult.fixed_songs.slice(0, 5).map(s => s.title).join(', ')}
                    {refreshResult.fixed_songs.length > 5 ? ` +${refreshResult.fixed_songs.length - 5} more` : ''}
                  </p>
                )}
              </div>
            )}
